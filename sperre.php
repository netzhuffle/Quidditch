			...
			echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\">$t[sonst3]</TR>\n";	
				...
				if (strlen($row->isip)>0):
					...
					if ($ip_name==$row->is_domain):
						unset($ip_name);
						echo "<TR VALIGN=TOP BGCOLOR=\"$bgcolor\"><TD><B>".$f1.stripslashes($row->is_domain).$f2."</B></TD>\n";
					else:
						echo "<TR VALIGN=TOP BGCOLOR=\"$bgcolor\"><TD><B>".$f1.stripslashes($row->is_domain)."<br>(".$ip_name.$f2.")</B></TD>\n";
					endif;
				endif;
				
				// Infotext
				echo "<TD>".$f1.stripslashes($row->is_infotext)."&nbsp;".$f2."</TD>\n";
...
if (strlen($u_id)>0 && $admin):
	...
	if ((isset($eintragen)) && ($eintragen==$t['sonst13'])):
		...
		if (!isset($f['is_domain'])) $f['is_domain'] = "";
		...
		if (strlen($is_id)>0):
			...
			if ($rows==1):
				...
				echo "<TD>".$f1.
					...
					"VALUE=\"$row->is_infotext\" SIZE=24>".$f2."</TD></TR>\n";

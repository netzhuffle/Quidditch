	...
	if ($result) {
		...
		if ($anzahl==0) {
			...
			echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\"><TD COLSPAN=5><DIV style=\"color:$farbe_text;\"><B>$titel: $anzahl</B></DIV></TD></TR>\n".
				...
				"<TD WIDTH=\"13%\" ALIGN=\"CENTER\">".$f1."Eintrag&nbsp;von".$f2."</TD></TR>\n";
			...
			while($row=mysql_fetch_object($result)) {
				...
				if ($result2 && mysql_num_rows($result2)>0) {
					...
					$blacklist_nick="<B>".user($row2->u_id,$row2,TRUE,FALSE)."</B>";
				...
				"<TD ALIGN=\"CENTER\">".$auf.$admin_nick.$zu."</TD>".
				"</TR>\n";
...
function formular_neuer_blacklist($neuer_blacklist) {
	...
	if (!isset($neuer_blacklist['u_nick'])) $neuer_blacklist['u_nick'] = "";
	if (!isset($neuer_blacklist['f_text'])) $neuer_blacklist['f_text'] = "";
	...
	echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\"><TD COLSPAN=2><DIV style=\"color:$farbe_text;\"><B>$titel</B></DIV></TD></TR>\n".
		...
		"<INPUT TYPE=\"TEXT\" NAME=\"neuer_blacklist[u_nick]\" VALUE=\"".htmlspecialchars(stripslashes($neuer_blacklist['u_nick']))."\" SIZE=20>".
		$f2."</TD></TR>\n".
		"<TR BGCOLOR=\"$farbe_tabelle_zeile1\"><TD align=\"right\"><B>Infotext:</B></TD><TD>".$f1.
		"<INPUT TYPE=\"TEXT\" NAME=\"neuer_blacklist[f_text]\" VALUE=\"".htmlspecialchars(stripslashes($neuer_blacklist['f_text']))."\" SIZE=$eingabe_breite>".
		"&nbsp;".

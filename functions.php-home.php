...
function edit_home($u_id,$u_nick,$home,$einstellungen,$farben,$bilder,$aktion) {
	...
	echo "<TABLE BORDER=\"0\" HEIGHT=\"100%\" WIDTH=\"100%\" BGCOLOR=\"$farbe_tabelle_zeile2\" CELLPADDING=5 CELLSPACING=0>".
		"<TR><TD COLSPAN=\"3\" BGCOLOR=\"$farbe_tabelle_zeile1\">$f1<B>Meine Homepage:</B>".$f2."</TD></TR>\n".
...
function home_profil($u_id,$u_nick,$home,$farben,$aktion) {
	...
	if ($home['ui_userid']) {
		...
		if (($ui_einstellungen["Ort"] || $ui_einstellungen["PLZ"]) && ($home['ui_plz'] || $home['ui_ort'])) {
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Ort:".$f2."</TD><TD colspan=3>";
			if ($ui_einstellungen["PLZ"]) $text.=htmlspecialchars(stripslashes($home['ui_plz']))."&nbsp;&nbsp;";
			if ($ui_einstellungen["Ort"]) $text.=htmlspecialchars(stripslashes($home['ui_ort']));
			$text.="</TD></TR>\n";
		}

		if ($ui_einstellungen["Land"] && $home['ui_land'])
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Land:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_land']))."</TD></TR>\n";

		if ($ui_einstellungen["Tel"] && $home['ui_tel']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Telefon:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_tel']))."</TD></TR>\n";

		if ($ui_einstellungen["Handy"] && $home['ui_handy']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Handy:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_handy']))."</TD></TR>\n";

		if ($ui_einstellungen["Fax"] && $home['ui_fax']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Telefax".$f2.":</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_fax']))."</TD></TR>\n";

		if ($ui_einstellungen["ICQ"] && $home['ui_icq']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."ICQ:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_icq']))."</TD></TR>\n";

		if ($ui_einstellungen["Geburtsdatum"] && $home['ui_geburt']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Geburtsdatum:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_geburt']))."</TD></TR>\n";

		if ($ui_einstellungen["Geschlecht"] && $home['ui_geschlecht']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Geschlecht:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_geschlecht']))."</TD></TR>\n";

		if ($ui_einstellungen["Beziehung"] && $home['ui_beziehung']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Beziehung:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_beziehung']))."</TD></TR>\n";

		if ($ui_einstellungen["Typ"] && $home['ui_typ']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Typ:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_typ']))."</TD></TR>\n";

		if ($ui_einstellungen["Beruf"] && $home['ui_beruf']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Beruf:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_beruf']))."</TD></TR>\n";

		if ($ui_einstellungen["Hobbies"] && $home['ui_hobby']) 
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1."Hobbies:".$f2."</TD><TD colspan=3>".
				htmlspecialchars(stripslashes($home['ui_hobby']))."</TD></TR>\n";
...
function home_info($u_id,$u_nick,$farben,$aktion) {
	...
	if ($result && mysql_num_rows($result)==1) {
		...
		$text=	"<TR><TD valign=\"TOP\" align=\"right\" WIDTH=20%>".$f1."Nickname:".$f2."</TD><TD colspan=3 WIDTH=\"80%\"><B>".
			user($userdata['u_id'],$userdata,$links_an,FALSE).
			"</B></TD></TR>\n";
		
		// Onlinezeit oder letzter Login		
		if ($userdata['o_id']!="NULL" && $userdata['o_id']) {
			$text.="<TR><TD>&nbsp;</TD><TD colspan=3 valign=\"TOP\"><B>".$f1.
				str_replace("%online%",gmdate("H:i:s",$online_zeit),$t['chat_msg92']).
				$f2."</B></TD></TR>\n";
		} else {
			$text.="<TR><TD>&nbsp;</TD><TD colspan=3 valign=\"TOP\"><B>".$f1.
		...
		// Level
		$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1.$t['user_zeige8'].":".$f2."</TD>".
			"<TD colspan=3><B>".$f1.$level[$userdata['u_level']].$f2."</B></TD></TR>\n";
		...
		if ($userdata['u_punkte_gesamt']) {
			...
			$text.="<TR><TD valign=\"TOP\" align=\"right\">".$f1.$t['user_zeige38'].":".$f2."</TD>".
				"<TD colspan=3><B>".$f1.
				$userdata['u_punkte_gesamt']."/".$userdata['u_punkte_jahr']."/".$userdata['u_punkte_monat']. 
				"&nbsp;".str_replace("%jahr%",strftime("%Y",time()),str_replace("%monat%",strftime("%B",time()),$t['user_zeige39'])).
				$f2."</B></TD></TR>\n";
		};
...
function home_bild($u_id,$u_nick,$home,$feld,$farben,$aktion,$bilder,$beschreibung="") {
	...
	if (is_array($bilder) && isset($bilder[$feld]) && $bilder[$feld]['b_mime']) {
		...
		if ($aktion=="aendern") {
			$text.="<TD VALIGN=\"BOTTOM\" ALIGN=\"RIGHT\">".
				$f3."<B>[<A HREF=\"$PHP_SELF?http_host=$http_host&id=$id&aktion=aendern&loesche=$feld\">L÷SCHEN</A>]</B>".$f4."<BR>".
				home_farbe($u_id,$u_nick,$home,$feld,$farben[$feld])."</TD>\n";
		} elseif ($aktion=="aendern_ohne_farbe") {
			$text.="<TD VALIGN=\"BOTTOM\" ALIGN=\"RIGHT\">".
				$f3."<B>[<A HREF=\"$PHP_SELF?http_host=$http_host&id=$id&aktion=aendern&loesche=$feld\">L÷SCHEN</A>]</B>".$f4."</TD>\n";
		};
	...
	if (!isset($farben[$feld])) {
		...
		$bg="BACKGROUND=\"home_bild.php?http_host=$http_host&u_id=$u_id&feld=".$farben[$feld]."\"";
	} elseif (strlen($farben[$feld])==7) {
	...
	if ($beschreibung) $text="<TD align=\"RIGHT\">".$f1.$beschreibung.$f2."</TD>".$text;
	if ($text && $beschreibung) {
		$text="<TR>".$text."</TR>";
	} elseif ($text) {
		$text="<TABLE $bg CELLPADDING=\"5\" CELLSPACING=\"0\"  WIDTH=\"100%\" BORDER=\"0\" ><TR>".$text."</TR></TABLE>";
	};
...
function home_aktionen($u_id,$u_nick,$home,$farben,$aktion) {
	...
	$text="<TABLE $bg CELLPADDING=\"5\" CELLSPACING=\"0\" BORDER=\"0\" WIDTH=\"100%\"><TR>".$text."</TR></TABLE>";
...
function home_hintergrund($u_id,$u_nick,$farben,$home,$bilder) {
	...
	$text="<TR><TD align=\"RIGHT\">".$f1."Hintergrund: ".$f2."</TD>".
		"<TD BGCOLOR=\"$farben[bgcolor]\">&nbsp;</TD>".
		"<TD>".home_farbe($u_id,$u_nick,$home,"bgcolor",$farben['bgcolor'])."</TD></TR>".
		"<TR><TD align=\"RIGHT\">".$f1."Textfarbe: ".$f2."</TD>".
		"<TD BGCOLOR=\"$farben[text]\">&nbsp;</TD>".
		"<TD>".home_farbe($u_id,$u_nick,$home,"text",$farben['text'],FALSE)."</TD></TR>".
		"<TR><TD align=\"RIGHT\">".$f1."Linkfarbe: ".$f2."</TD>".
		"<TD BGCOLOR=\"$farben[link]\">&nbsp;</TD>".
		"<TD>".home_farbe($u_id,$u_nick,$home,"link",$farben['link'],FALSE)."</TD></TR>".
		"<TR><TD align=\"RIGHT\">".$f1."Linkfarbe (aktiv): ".$f2."</TD>".
		"<TD BGCOLOR=\"$farben[vlink]\">&nbsp;</TD>".
		"<TD>".home_farbe($u_id,$u_nick,$home,"vlink",$farben['vlink'],FALSE)."</TD></TR>"
		.
		home_bild($u_id,$u_nick,$home,"ui_bild4",$farben,$aktion,$bilder,"Hintergrundgrafik&nbsp;1:").
		home_bild($u_id,$u_nick,$home,"ui_bild5",$farben,$aktion,$bilder,"Hintergrundgrafik&nbsp;2:").
		home_bild($u_id,$u_nick,$home,"ui_bild6",$farben,$aktion,$bilder,"Hintergrundgrafik&nbsp;3:")
		;
...
function home_einstellungen($u_id,$u_nick,$home,$einstellungen) {
	...
	foreach ($vor_einstellungen as $key => $val) {
		...
		$text.="<TR><TD align=\"RIGHT\">".$f1.$key." zeigen:".$f2."</TD>".
...
function home_farbe($u_id,$u_nick,$home,$feld,$farbe="#FFFFFF",$mit_grafik=TRUE) {
	...
	$url="home_farben.php?http_host=$http_host&id=$id&mit_grafik=$mit_grafik&feld=$feld&bg=Y&oldcolor=".urlencode($farbe);
	$link=$f3."<B>[<A HREF=\"$url\" TARGET=\"Farben\" onclick=\"window.open('$url','Farben','resizable=yes,scrollbars=yes,width=400,height=500'); return(false);\">FARBE</A>]</B>".$f4;
	return($link);
...
function zeige_home($u_id,$force=FALSE,$defaultfarben="") {
	...
	global $dbase,$conn,$argv,$argc,$ivw,$http_host,$id,$homepage_extern,$check_name;
	...
	if ($u_id) {
		...
		if (!isset($u_chathomepage)) $u_chathomepage = 'N';
		if (!$force && $u_chathomepage!="J") $ok=FALSE;
	...
	if (isset($farben['bgcolor']) && strlen($farben['bgcolor'])>7) {
		$bg="BACKGROUND=\"home_bild.php?http_host=$http_host&u_id=$u_id&feld=".$farben['bgcolor']."\"";

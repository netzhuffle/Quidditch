...
<!--
<input type=hidden name="addr_name" value="'.stripslashes($userdata['u_name']).'">
...
if ($u_id && $communityfeatures) {
	...
	$result=mysql_query($q) or trigger_error(mysql_error(), E_USER_ERROR);
	...
	$u=unserialize($userdata['u_sms_extra']);
	 ...
	 if ($aktion == "pinsenden" && ( $u['pintime'] + 86400 < date("U") || $u['pintime'] == "") )
		...
		sms_sende2($f['ui_handy'],"Hallo! Die Auflade-PIN fuer den $chat lautet: $u[pin]");
		unset($g);
...
$errmsg="";
 if (isset($pin))
		...
		if ($u[pin] == $pin && ($u[pinversuche] <= 4))
			...
			unset($stuff);
			while(list($http_stuff_name,$http_stuff_inhalt)=each($http_stuff))
				if ($http_stuff_inhalt) $stuff.="$http_stuff_name:\t$http_stuff_inhalt\n";
	...
	echo "<IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=4><BR>\n";
		...
		if (($u['pin'] != "") && ( $u['pintime'] + 86400 > date("U")) && ($u['pinversuche'] <= 4) && ($u['handynrok']==false) ) { PinEingabeFormular($errmsg);}

		// Wenn Fehleingaben > 4 dann Fehlermeldung
		if (($u['pin'] != "") && ( $u['pintime'] + 86400 > date("U")) && ($u['pinversuche'] > 4) && ($u['handynrok']==false) ) { PinErrorFormular();}

...
function formular_neue_email($neue_email,$m_id="") {
	...
	if (!isset($neue_email['an_nick'])) $neue_email['an_nick'] = "";
	echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\"><TD COLSPAN=2><DIV style=\"color:$farbe_text;\"><B>$titel</B></DIV></TD></TR>\n".
		...
		"<INPUT TYPE=\"TEXT\" NAME=\"neue_email[an_nick]\" VALUE=\"".stripslashes($neue_email['an_nick'])."\" SIZE=20>".
		"&nbsp;".
...
function formular_neue_email2($neue_email,$m_id="") {
	...
	if ($u_punkte_gesamt > $sms['punkte'] && $smsgh > 0) $darfsms=true; else $darfsms=false;

	if (!$eingabe_breite1) $eingabe_breite1=30;
	if (!$eingabe_breite2) $eingabe_breite2=40;
	...
	if ($neue_email['m_an_uid']) {
		...
		if ($row->o_id=="" || $row->o_id=="NULL") $row->online="";

		if (!isset($neue_email['m_betreff'])) $neue_email['m_betreff'] = ""; 
		echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\"><TD COLSPAN=3><DIV style=\"color:$farbe_text;\"><B>$titel ".user($row->u_id,$row,TRUE,TRUE,"&nbsp;",$row->online,$row->login)."</B>$email</DIV></TD></TR>\n".
				"<TR BGCOLOR=\"$farbe_tabelle_zeile2\"><TD VALIGN=\"TOP\" align=\"right\"><B>Betreff:</B></TD><TD  COLSPAN=2>".$f1.
				"<INPUT TYPE=\"TEXT\" NAME=\"neue_email[m_betreff]\" VALUE=\"".stripslashes($neue_email['m_betreff'])."\" SIZE=".($eingabe_breite1)."
				 ONCHANGE=zaehle() ONFOCUS=zaehle() ONKEYDOWN=zaehle() ONKEYUP=zaehle()>".
				$f2."<input name=\"counter\" size=3></TD></TR>".
				"<TR BGCOLOR=\"$farbe_tabelle_zeile1\"><TD VALIGN=\"TOP\" align=\"right\"><B>Ihr Text:</B></TD><TD COLSPAN=2>".$f1.
				"<TEXTAREA COLS=".($eingabe_breite2)." ROWS=20 NAME=\"neue_email[m_text]\" ONCHANGE=zaehle() ONFOCUS=zaehle() ONKEYDOWN=zaehle() ONKEYUP=zaehle()>".stripslashes($neue_email['m_text'])."</TEXTAREA>\n".
				$f2."</TD></TR>".
				"<TR BGCOLOR=\"$farbe_tabelle_zeile2\"><TD>&nbsp;</TD><TD>$email_select</TD>".
				"<TD ALIGN=\"right\" >".$f1."<INPUT TYPE=\"SUBMIT\" NAME=\"los\" VALUE=\"VERSENDEN\">".
				$f2."</TD></TR>\n".
				"</TABLE></FORM>\n";
	...
	if ($result) {
		...
		if ($anzahl==0) {
			...
			while($row=mysql_fetch_object($result)) {
				...
				$url="<A HREF=\"".$PHP_SELF."?id=$id&http_host=$http_host&aktion=zeige&m_id=".$row->m_id."\">";
				...
				"<INPUT TYPE=\"CHECKBOX\" NAME=\"loesche_email[]\" VALUE=\"".$row->m_id."\"></TD>".
				"<TD>".$auf.$url.$von_nick."</A>".$zu."</TD>".
				"<TD WIDTH=\"50%\">".$auf.$url.htmlspecialchars(stripslashes($row->m_betreff))."</A>".$zu."</TD>".
				"<TD>".$auf.$row->zeit.$zu."</TD>".
				"<TD>".$auf.$row->m_status.$zu."</TD>".
				"</TR>\n";
...
function zeige_email($m_id) {
	...
	if ($result && mysql_num_rows($result)==1) {
		...
		echo "<TR BGCOLOR=\"$farbe_tabelle_kopf2\"><TD COLSPAN=4><DIV style=\"color:$farbe_text;\"><B>Mail zeigen:</B></DIV></TD></TR>\n".
			"<TR BGCOLOR=\"$farbe_tabelle_zeile1\"><TD ALIGN=\"RIGHT\"><B>".$f1."Von:".$f2."</B></TD><TD COLSPAN=3 WIDTH=\"100%\">".$von_nick."</TD></TR>\n".
			"<TR BGCOLOR=\"$farbe_tabelle_zeile2\"><TD ALIGN=\"RIGHT\"><B>".$f1."Am:".$f2."</B></TD><TD COLSPAN=3>".$f1.$row->zeit.$f2."</TD></TR>\n".
			"<TR BGCOLOR=\"$farbe_tabelle_zeile1\"><TD ALIGN=\"RIGHT\"><B>".$f1."Betreff:".$f2."</B></TD><TD COLSPAN=3>".$f1.htmlspecialchars(stripslashes($row->m_betreff)).$f2."</TD></TR>\n".
		...
		echo "<TR BGCOLOR=\"$farbe_tabelle_zeile1\"><TD>&nbsp;</TD>".
			...
			"<INPUT TYPE=\"HIDDEN\" NAME=\"http_host\" VALUE=\"$http_host\">\n".
			"<INPUT TYPE=\"SUBMIT\" NAME=\"los\" VALUE=\"ANTWORTEN\">".
			$f2."</TD></FORM>\n".
			"<FORM NAME=\"mail_antworten\" ACTION=\"$PHP_SELF\" METHOD=POST>\n".
			"<TD ALIGN=\"center\">".$f1.
			"<INPUT TYPE=\"HIDDEN\" NAME=\"id\" VALUE=\"$id\">\n".
			...
			"<INPUT TYPE=\"HIDDEN\" NAME=\"aktion\" VALUE=\"loesche\">\n".
			"<INPUT TYPE=\"HIDDEN\" NAME=\"http_host\" VALUE=\"$http_host\">\n".
			"<INPUT TYPE=\"SUBMIT\" NAME=\"los\" VALUE=\"L÷SCHEN\">".
			$f2."</TD></FORM></TR>\n";

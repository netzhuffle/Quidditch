	...
	if ( isset($farbe) ) 
	{
	    if (strlen($farbe) == 6 )
	    {
	        if (preg_match("/[a-f0-9]{6}/i", $farbe))
	        {
	        	...
	        	system_msg("",0,$u_id,$system_farbe,"<B>$chat:</B> ".
			    "<FONT COLOR=\"#$u_farbe\">".str_replace("%u_nick%",$u_nick,$t['farbe1'])."</FONT>");
                }
            }
	}
...
<?php 
	...
	$body_tag=$body_tag."TEXT=\"$farbe_chat_text2\" ".
                "LINK=\"$farbe_chat_link2\" ".
                "VLINK=\"$farbe_chat_vlink2\" ".
	...
	if ($u_level=="M") {
		...
		$text2_typ="<INPUT TYPE=\"TEXT\" NAME=\"text2\" maxlength=\"".($chat_max_eingabe-1)."\" VALUE=\"\" SIZE=\"".$chat_eingabe_breite."\">";
	};
	...
	if ($backup_chat || $u_backup) {
		...
		echo $text2_typ.
			...
			"<SELECT NAME=\"user_chat_back\">\n";
                for($i=5; $i<40; $i++)  echo "<OPTION ".
                                               ($chat_back==$i?"SELECTED":"").

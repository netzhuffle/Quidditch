...
function user_liste($larr,$anzahl) {
		...
		if (!isset( $larr[0]['r_name'] ))  $larr[0]['r_name'] = "";
		if (!isset( $larr[0]['r_besitzer'] ))  $larr[0]['r_besitzer'] = "";
		if (!isset( $larr[0]['r_topic'] ))  $larr[0]['r_topic'] = "";
		...
		$r_topic    = $larr[0]['r_topic'];
                $level      = "user";
                $level2     = "user";
		if ( $r_besitzer==$u_id && $adminfeatures)         $level = "owner";
		if (( $admin || $u_level=="A" ) && $adminfeatures) $level = "admin";
		if (($u_level == "C" || $u_level =="S"  ) && $adminfeatures) $level2 = "admin";

		#system_msg("",0,$u_id,"","DEBUG: u_level = $u_level");

		// Wartetext ausgeben
		if ($anzahl>10) echo $f1."<B>$t[sonst23] $anzahl $t[sonst36]</B><BR>\n".$f2; flush();

                if ( $o_js ) {
                
                    if ( $show_geschlecht == true ) {
                        $geschl = array();
                        foreach ( $larr as $k => $v )    $ids[] = $v['u_id'];
			...
			for ($k=0; isset($larr[$k]) AND is_array($larr[$k]) AND $v=$larr[$k]; $k++ ) {
				if ( $level!="admin" ) {
					...
					$v['o_ip']="";
				      ...
				      if (!isset($v['hostname'])) $v['hostname'] = "";				
                                      $v['hostname']=htmlspecialchars($v['hostname']);
				      if (!isset($v['o_ip'])) $v['o_ip'] = "";				
				};
				$v['u_away'] = $v['u_away'] ? "y" : "";
				if (!isset($v['u_home_ok'])) $v['u_home_ok'] = "";
				if ( $show_geschlecht == true ) {
...
//			     "   stdparm=''; stdparm2=''; id=''; http_host=''; u_nick=''; raum=''; nlink=''; nick=''; url='';\n".
	...
	echo "<SCRIPT LANGUAGE=\"JavaScript\" src=\"popup.js\"></script>\n";
			...
			for ($k=0; is_array($larr[$k]) AND $v=$larr[$k]; $k++ ) {
				...
				$trow   .= "<TR BGCOLOR=\"".($k&1?$farbe_tabelle_zeile1:$farbe_tabelle_zeile2)."\">".
					   "<TD>".$f1.$user.$f2."</TD></TR>";
			} // END-OF-FOR
...
global $chat_grafik,$whotext,$beichtstuhl,$erweitertefeatures,$msgpopup,$serverprotokoll;
		...
		if ($result AND mysql_num_rows($result)==1):
			...
			$uu_away=str_replace("\\","",htmlspecialchars(stripslashes($row->u_away)));
			$uu_nick=str_replace("\\","",htmlspecialchars(stripslashes($row->u_nick)));
			$uu_name=str_replace("\\","",htmlspecialchars(stripslashes($row->u_name)));
			$uu_id=$row->u_id;
			$uu_email=str_replace("\\","",htmlspecialchars(stripslashes($row->u_email)));
			$uu_adminemail=str_replace("\\","",htmlspecialchars(stripslashes($row->u_adminemail)));
			$uu_url=str_replace("\\","",htmlspecialchars(stripslashes($row->u_url)));
			...
			if ($result && $rows=mysql_num_rows($result)==1):
				...
				if (isset($o_http_stuff)) $http_stuff=unserialize($o_http_stuff);
			endif;
			...
			if (isset($onlinezeit) && $onlinezeit && $u_level!="G"):
				...
				if ($msgpopup)
				{
					...
					print '<iframe src="messages-popup.php?id='.$id.'&http_host='.$http_host.'&user='.$user.'&user_nick='.$uu_nick.'" width=100% height=200 marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0></iframe>';
					print "<IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=4><BR>\n";	
					# echo("DEBUG: $user $uu_nick <br>\n");
				}


	  			echo 	"<INPUT NAME=\"text2\" SIZE=\"".$eingabe_breite."\" maxlength=\"".($chat_max_eingabe-50)."\" VALUE=\"\" TYPE=\"TEXT\">".
			...
			echo "<TR><TD VALIGN=TOP><B>".$f1.$t['user_zeige18'].$f2."</B></TD><TD><B>".
				user($user,$row,TRUE,FALSE);
			...
			if ($admin):
				// Name
				if (strlen($uu_name)>0):
					echo "<TR><TD><B>".$f1.$t['user_zeige2'].$f2."</B></TD><TD><B>".$f1.
						"$uu_name".$f2."</B></TD></TR>\n";
				endif;
			endif;
			...
			if (isset($o_row) && $o_row->r_name && $o_row->o_who==0):
				echo "<TR><TD><B>".$f1.$t['user_zeige23'].$f2."</B></TD><TD><B>".$f1.
					$o_row->r_name."&nbsp;[".$whotext[$o_row->o_who]."]".$f2."</B></TD></TR>\n";
			elseif(isset($o_row) && $o_row->o_who):
				echo "<TR><TD>".$f1."&nbsp;".$f2."</TD>".
					"<TD><B>".$f1."[".$whotext[$o_row->o_who]."]".$f2."</B></TD></TR>\n";
			endif;
			if (isset($onlinezeit) && $onlinezeit):
				echo "<TR><TD>".$f1.$t['user_zeige33'].$f2."</TD><TD VALIGN=BOTTOM>".$f3.
					gmdate("H:i:s",$onlinezeit)."&nbsp;".$t['sonst27'].$f4."</TD></TR>\n";
			else:
				echo "<TR><TD>".$f1.$t['user_zeige9'].$f2."</TD><TD VALIGN=TOP>".$f1.
					"$letzter_login".$f2."</TD></TR>\n";
			endif;

			if ($erster_login && $erster_login!="01.01.1970 01:00"):
				echo "<TR><TD>".$f1.$t['user_zeige32'].$f2."</TD><TD VALIGN=TOP>".$f1.
					"$erster_login".$f2."</TD></TR>\n";
			endif;
			...
			if ($communityfeatures && $uu_punkte_gesamt) {
				...
				echo "<TR><TD>".$f1.$t['user_zeige38'].$f2."</TD><TD VALIGN=TOP>".$f3.
					$uu_punkte_gesamt."/".$uu_punkte_jahr."/".$uu_punkte_monat.
					"&nbsp;".str_replace("%jahr%",substr(strftime("%Y",time()),2,2),str_replace("%monat%",substr(strftime("%B",time()),0,3),$t['user_zeige39'])).
					$f4."</TD></TR>\n";
			};
			...
			if ($admin){
				...
				if (strlen($uu_adminemail)>0){
					echo "<TR><TD>".$f1.$t['user_zeige3'].$f2."</TD><TD>".$f3.
						"<A HREF=\"MAILTO:$uu_adminemail\">$uu_adminemail</A>".$f4."</TD></TR>\n";
				};
			};
			...
			$fenster=str_replace("ß","",$fenster);
				...
				echo "<TR><TD>".$f1.$t['user_zeige6'].$f2."</TD><TD>".$f3.
					"<A HREF=\"MAILTO:$uu_email\">$uu_email</A>".$f4."</TD></TR>\n";
			};
			...
			if ($communityfeatures && $uu_chathomepage=="J") {
				...
				echo "<TR><TD>".$f1.$t['user_zeige7'].$f2."</TD><TD>".$f3.
					"<A HREF=\"$uu_url\" TARGET=\"_new\">$uu_url</A>".$f4."</TD></TR>\n";
			};
			...
			if ($admin):
				...
				if (isset($o_row) && $o_row->o_ip):
					echo "<TR><TD VALIGN=TOP>".$f1.$t['user_zeige4'].$f2."</TD><TD>".$f3.
						$host_name.$f4."</TD></TR>\n".
						"<TR><TD VALIGN=TOP>".$f1."IP".$f2."</TD><TD>".$f3.
						$o_row->o_ip." ".$t['sonst28'].$f4."</TD></TR>\n";

					if ($zeigeip==1 && is_array($ip_historie)):
						while(list($datum,$ip_adr)=each($ip_historie)):
							echo "<TR><TD></TD><TD>".$f3.
								$ip_adr."&nbsp;(".str_replace(" ","&nbsp;",date("d.m.y H:i",$datum)).")".$f4."</TD></TR>\n";
						endwhile;
					endif;

					echo "<TR><TD VALIGN=TOP>".$f1.$t['user_zeige5'].$f2."</TD><TD>".$f3.
						htmlspecialchars($o_row->o_browser).$f4."</TD></TR>\n".
						"<TR><TD VALIGN=TOP>".$f1.$t['user_zeige22'].$f2."</TD><TD>".$f3.
						"<a href=\"".$serverprotokoll."://".$o_row->o_vhost."\" target=_new>$serverprotokoll://".$o_row->o_vhost."</a>".$f4."</TD></TR>\n";

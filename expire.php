...
if ($rows>0):
	...
	while($i<$rows):
		...
		if ($i>0) $loesche.=",";
		if (!$row->r_name) $row->r_name="_ohne_Raum";

		// Chat-Zeile in Log schreiben
		$r_name=$log."/".str_replace("/","_",$row->r_name);
                if ($raum_alt!=$r_name) echo "  Raum $r_name\n";
	...
	$result3=mysql_query($query,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
endif;
...
if ($expire_privat):
	...
	if ($rows>0):
		...
		while($i<$rows):
			...
			// Chat-Zeile löschen
			$result3=mysql_query("DELETE FROM chat WHERE c_id=$row->c_id",$conn) or trigger_error(mysql_error(), E_USER_ERROR);
			$i++;
		endwhile;
		mysql_free_result($result);
	endif;
...
if ($result && mysql_num_rows($result)>0):
	while($row=mysql_fetch_object($result)):
		...
		// User löschen
		$result3=mysql_query("DELETE FROM user WHERE u_id=$row->u_id",$conn) or trigger_error(mysql_error(), E_USER_ERROR);

		// User-Ignore Einträge löschen
		$result3=mysql_query("DELETE FROM iignore WHERE i_user_aktiv=$row->u_id OR i_user_passiv=$row->u_id",$conn) or trigger_error(mysql_error(), E_USER_ERROR);

		// Gesperrte Räume löschen
		$query3="DELETE FROM sperre WHERE s_user=$row->u_id"; $result3=mysql_query($query3,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	...
	while($row=mysql_fetch_object($result)):
		...
		if ($result3):
			if (mysql_num_rows($result3)==0):
				...
				$result4=mysql_query($query,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
...
if ($zeit=="03:10") {
	...
	if (!isset($nicknamen_expire) || $nicknamen_expire == 0) { $nicknamen_expire=15724800; }
	...
	if (strlen($besitzer)>0)
	{
		$query="SELECT SQL_BUFFER_RESULT u_id, u_nick, u_login from user where (UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(u_login)) > ".$nicknamen_expire." " 
		      ." AND u_level!='Z' AND u_level!='S' ".$besitzer;
	}
	else
	{
		$query="SELECT SQL_BUFFER_RESULT u_id, u_nick, u_login from user where (UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(u_login)) > ".$nicknamen_expire." " 
		      ." AND u_level!='Z' AND u_level!='S'";
	}	
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM bild WHERE b_id=".$row->b_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM mail WHERE m_id=".$row->m_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM aktion WHERE a_id=".$row->a_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM userinfo WHERE ui_id=".$row->ui_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);

	// alle User aus der Blacklist löschen, die es nicht mehr gibt.
	echo " Blacklist "; flush();
	$query="SELECT SQL_BUFFER_RESULT f_id FROM blacklist LEFT JOIN user on f_blacklistid=u_id WHERE u_id IS NULL;";  
	$result = mysql_query($query,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM blacklist WHERE f_id=".$row->f_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM iignore WHERE i_id=".$row->i_id,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	mysql_free_result($result);
	...
	sleep(60);
	set_time_limit(600);
	echo " Repariere user,raum "; flush();
        $query="REPAIR TABLE user,raum";
	mysql_query($query,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
	sleep(60);
	echo " fertig<BR>"; flush();
};
	...
	if ((strlen($STAT_DB_HOST) > 0))
	{
		...
		{
		    $r0 = @mysql_query("SELECT '$STAT_DB_COLLECT'",$conn) or trigger_error(mysql_error(), E_USER_ERROR);
		}
		else
		{
		    $r0 = @mysql_query("SELECT SQL_BUFFER_RESULT DISTINCT o_vhost FROM online WHERE o_raum>0",$conn) or trigger_error(mysql_error(), E_USER_ERROR);
		}
		...
		if ($r0 > 0)
		{
			...
			if ($n > 0)
			{
				...
				while ($i < $n)
				{
					...
					if ($r1 > 0)
					{
						$onlinevhosts["$o_vhost"] = intval(@mysql_result($r1, 0, "anzahl"));
					}

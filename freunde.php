...
if ($admin) $text.="| <A HREF=\"freunde.php?http_host=$http_host&id=$id&aktion=admins\">Alle Admins als Freund hinzufügen</A>\n";
	...
	case "neu2":
		...
		if ($result && mysql_num_rows($result)==1) 
		{
			...
			if ($ignore)
			{
				echo(str_replace("%u_name%",$neuer_freund['u_nick'],$t['chat_msg116']));
				formular_neuer_freund($neuer_freund);
			}
			else if ($neuer_freund['u_level'] == 'Z')
			{
				echo(str_replace("%u_name%",$neuer_freund['u_nick'],$t['chat_msg117']));
				formular_neuer_freund($neuer_freund);
			}
			else if ($neuer_freund['u_level'] == 'G')
			{
				echo(str_replace("%u_name%",$neuer_freund['u_nick'],$t['chat_msg118']));
				formular_neuer_freund($neuer_freund);
			}
			else
			{

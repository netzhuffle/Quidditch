...
//	echo "DEBGUG: $query2";
	...
	case "neu3":
		...
		if ($ok) {
			...
			if ($neue_email['typ']==1) {
				...
				$smszulang=false;
				$text=$neue_email['m_betreff']." ".$neue_email['m_text'];
				$emp=user($u_id,$userdata,FALSE,FALSE,"&nbsp;","","",FALSE,TRUE);
				...
				$result=mail_sende($u_id,$neue_email['m_an_uid'],$neue_email['m_text'],$neue_email['m_betreff']);
				if ($result[0]) {
	...
	case "antworten_forum":
		...
		if ($row->u_nick && $row->u_nick!="NULL") {
			...
			$neue_email['m_text']=erzeuge_quoting($neue_email['m_text'],$row->u_nick,$row->po_date);
			$neue_email['m_text']=erzeuge_fuss($neue_email['m_text']);

<?php
	...
	// Wandelt eine Background-Variable in den entsprechenden HTML-Source (bgcolor, background) um.
	{
		...
		if (("$backc" == "") || ("$backp" == ""))
		{
			if ("$backc" != "") return(" BGCOLOR=\"$backc\"");
			if ("$backp" != "") return(" BACKGROUND=\"$backp\"");
		}
	...
	Function statsOverview($v = "%")//{{{
	{
		...
		$r1 = @mysql_query("SELECT DISTINCT c_host FROM chat WHERE c_timestamp LIKE '$y$m%' AND c_host LIKE '$v' ORDER BY c_host") or trigger_error(mysql_error(), E_USER_ERROR);
		...
		if ($r1 > 0)
		{
			...
			if ($o > 0)
			{
				...
				while ($j < $o)
				{
					...
					$r0 = @mysql_query("SELECT * FROM chat WHERE c_timestamp LIKE '$y$m%' AND c_host='" . AddSlashes($c_host) or trigger_error(mysql_error(), E_USER_ERROR) . "' ORDER BY c_timestamp");
					...
					if ($r0 > 0)
					{
						...
						if ($c_host == "") $c_host = $STAT_TXT["0080"];
		...
		if ((isset($grapharray)) && (count($grapharray) > 0))
		{
			...
			while (list($i, $v) = each($grapharray))
			{
				if ($v > 0)
				{
					...
					if (strlen($STAT_BAR_IMAGE_T) > 0) $msg.="<IMG " . coreMakeImage($STAT_BAR_IMAGE_T) . "><BR>";

					$msg.="<IMG SRC=\"$img_p\" BORDER=\"0\" WIDTH=\"$img_w\" HEIGHT=\"$s\">";
					$msg.="<BR>";

					if (strlen($STAT_BAR_IMAGE_B) > 0) $msg.="<IMG " . coreMakeImage($STAT_BAR_IMAGE_B) . "><BR>";

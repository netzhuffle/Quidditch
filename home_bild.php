...
if (file_exists($cachepfad)) 
{
	...
	if (is_array($image))
	{
		...
		switch ($image[2])
		{
			case 1: $image[2] = "image/gif"; break;
			case 2: $image[2] = "image/jpeg"; break;
			case 3: $image[2] = "image/png"; break;
			case 4: $image[2] = "application/x-shockwave-flash"; break;
			default: $image[2] = "";			
		}
		...
		if ($result && mysql_num_rows($result)==1) 
		{
			...
			if (($image[0] == $b_width) && ($image[1] == $b_heigth) && ($image[2] == $b_mime))
			{
...
if ($anzeigeauscache)
{
	...
	if (!@stat($cache)) mkdir($cache,0777);
	if (!@stat($cache."/".$http_host)) mkdir($cache."/".$http_host,0777);
	if (!@stat($cache."/".$http_host."/".substr($u_id,0,2))) mkdir($cache."/".$http_host."/".substr($u_id,0,2),0777);

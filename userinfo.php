...
if ($_SERVER['QUERY_STRING']=="hilfe") {
	...
	switch($art) {
				...
				$i = 0;
			        ...
			        if ($result && mysql_num_rows($result)>0) {
			                while ($row=mysql_fetch_object($result)):
			                        if ((!isset($eintritt) AND $row->r_name==$eintrittsraum) || (isset($eintritt) AND $row->r_id==$eintritt)) {
			                                $raeume=$raeume."<OPTION SELECTED VALUE=\"$row->r_id\">$row->r_name\n";
			                        } else {
			                                $raeume=$raeume."<OPTION VALUE=\"$row->r_id\">$row->r_name\n";
			                        }
			                        $i++;
        			        endwhile;
			        }
		...
		case "alles":
			...
			if (!$beichtstuhl) {
				...
				$ergebnis=array();
        			...
        			if ($abweisen==false and !$beichtstuhl and $ergebnis['online']>0) {
				        ...
				        $zeigen_alt=TRUE;
			                while ($row=mysql_fetch_object($result2)){
			                        ...
			                        if (($row->o_level=='S') or ($row->o_level=='C')) {
                        			        $nick=str_replace(';','','<b class="admin">'.$row->o_name.'</b>');
			                        } else {
		...
		if (isset($anzahl)) {
			...
			if (!isset($registriert)) $registriert = "";
			if (!isset($null)) $null = "";
			...
			if ($text!="j" && $jscript!="j" ) {
				...
				// Voreinstellungen
				if (!isset($hrot)) { $hrot=255; }
				if (!isset($hgruen)) { $hgruen=255; } 
				if (!isset($hblau)) { $hblau=255; }
				if (!isset($vrot)) { $vrot=0; }
				if (!isset($vgruen)) { $vgruen=0; }
				if (!isset($vblau)) { $vblau=0; }

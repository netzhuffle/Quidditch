...
function kopf_forum($admin) {
	...
	echo "<center><table width=\"760\" cellspacing=\"0\" cellpadding=\"3\" border=\"0\">\n";
        echo "<tr><td><B><a href=\"forum.php?id=$id&http_host=$http_host\">".$chat."-Forum</A>".$lnk[1]."</b></td>\n";
        echo "</table>\n";
...
function maske_forum($fo_id=0) {
	...
	#echo "fo_admin: $fo_admin<BR>binär:".decbin($fo_admin)."<BR>";
        ...
        # echo "<tr><td>$f1 <DIV style=\"color:$farbe_text; font-weight:bold;\">$t[forum_msg2]</DIV> $f2</td>";
        # echo "<td><select size=\"1\" name=\"fo_admin\">";
        # echo "<option value=\"0\">$t[forum_yes]\n";
        # echo "<option";
	# if ($fo_admin==1)
	#	echo " selected";
...
function forum_liste() {
	...
	#if (!$forum_admin)
	#	$sql .= "and fo_admin = 0 ";
	if ($u_level == "G") $sql.="and ( ((fo_admin & 8) = 8) or fo_admin = 0) ";
	...
	if ($query && mysql_num_rows($query)>0) {
            while ($thema = mysql_fetch_array($query, MYSQL_ASSOC)) {
                ...
                if ($fo_id_last != $thema['fo_id']) {
                        if ($zeile > 0) echo "</table></td></tr></table><br>";
                        echo "<table width=\"760\" cellspacing=\"0\" cellpadding=\"1\" border=\"0\" bgcolor=\"$farbe_tabellenrahmen\"></tr><td>\n";
			...
			echo "<tr><td>&nbsp;<DIV style=\"color:$farbe_text; font-weight:bold;\">&nbsp;&nbsp;" . htmlspecialchars(stripslashes($thema['fo_name'])) . "<a name=\"".$thema['fo_id']."\"></a></DIV></td>";
                        if ($forum_admin) 
			{
				...
				echo "<td width=\"17\"><a style=\"color:$farbe_text;\" href=\"forum.php?id=$id&http_host=$http_host&fo_id=$thema[fo_id]&fo_order=$thema[fo_order]&aktion=forum_up\">".$chat_grafik['forum_pfeil_oben']."</a><br><img src=\"pics/fuell.gif\" width=\"1\" height=\"1\" border=\"0\"><br><a href=\"forum.php?id=$id&http_host=$http_host&fo_id=$thema[fo_id]&fo_order=$thema[fo_order]&aktion=forum_down\">".$chat_grafik['forum_pfeil_unten']."</a></td>\n";
			}
                        echo "</tr></table></td>\n";
		...
		if ($zeile%2)
			...
			#$ungelesene = 99;
                        ...
                        if ($forum_admin) {
                                echo "<td width=\"550\">";
                                echo "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
                                echo "<tr><td width=\"385\">$f1<B><A href=\"forum.php?id=$id&http_host=$http_host&th_id=$thema[th_id]&aktion=show_thema\">" . htmlspecialchars(stripslashes($thema['th_name'])) . "</a></B>$f2<br>".
					$f3." ". htmlspecialchars(stripslashes($thema['th_desc'])) . "$f4</td>";
				...
				echo "<td width=\"85\" align=\"center\" valign=\"middle\"><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$thema[th_id]&aktion=thema_edit\">$chat_grafik[forum_themabearbeiten]</a>&nbsp;</td>";
                                echo "<td width=\"20\" align=\"center\"><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$thema[th_id]&fo_id=$thema[fo_id]&th_order=$thema[th_order]&aktion=thema_up\">".$chat_grafik['forum_pfeil_oben']."</a><br><img src=\"pics/fuell.gif\" width=\"1\" height=\"1\" border=\"0\"><br><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$thema[th_id]&fo_id=$thema[fo_id]&th_order=$thema[th_order]&aktion=thema_down\">".$chat_grafik['forum_pfeil_unten']."</td></tr></table></td>\n";

                        } else {

                                echo "<td width=\"550\">$f1<B><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$thema[th_id]&aktion=show_thema\">" . htmlspecialchars(stripslashes($thema['th_name'])) . "$f2</a></B><br>".
					$f3." ". htmlspecialchars(stripslashes($thema['th_desc'])) . "$f4</td>\n";
...
function show_icon_description($mode) {
        ...
        /*
	if ($mode == "thema")
                echo "(<img src=\"pics/forum/hot_folder.gif\" width=\"13\" height=\"16\" border=\"0\"> = $t[desc_burningfolder])";
        */
	echo "$f4</td></tr>";
...
function maske_thema($th_id=0) {
        ...
        if (!isset($th_name)) $th_name = "";
        if (!isset($th_desc)) $th_desc = "";
	...
	if ($th_id > 0) 
	{
		...
		echo "<input type=\"checkbox\" name=\"th_forumwechsel\" value=\"Y\">"; 
        	...
        	{
                	echo "<OPTION ";
                	if ($row->fo_id == $fo_id) echo "SELECTED ";
...
function show_pfad($th_id) {
	...
	if (!$anzahl_po_seite || $anzahl_po_seite==0) $anzahl_po_seite=20;
...
function show_thema() {
	...
	if ($anzahl_po_seite2) 
				...
				#print "anzahl_po_seite2=$anzahl_po_seite2<p>"; 
				...
				if (schreibe_db("user",$f,$u_id,"u_id")) 
					{
					#print "Schreiben in DB ok!";
					}
					else
					{
					print "Fehler beim Schreiben in DB!";
					}
	...
	echo "</tr></table></td>\n";
        #echo "<td width=\"100\" align=\"center\">$f3<a style=\"color:$farbe_text;\" href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_vater_id=0&aktion=thread_neu\">$t[neuer_thread]</a>$f4</td></tr></table></td>\n";
	...
	set_time_limit(0);
		...
		if ($ungelesene===0)
		{
			...
			else if ($posting['po_threadgesperrt']=='Y') // Geschlossener Thread
			{
		...
		#$folder=htmlentities($folder);
                ...
                {
                    echo "<td>&nbsp;<b><font size=\"-1\" color=\"$farbe_link\">".substr((stripslashes($posting['po_titel'])),0,40)."</font> <font size=\"-1\" color=\"red\">(gesperrt)</font></b></td>\n";
                }
                else if ($posting['po_gesperrt'] == 'Y' and $forum_admin)
                {
                    echo "<td>&nbsp;$f1<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=$posting[po_id]&thread=$posting[po_id]&aktion=show_posting&seite=$seite\">".substr((stripslashes($posting['po_titel'])),0,40)."</a></b>$f2  <font size=\"-1\" color=\"red\"><b>(gesperrt)</b></font></td>\n";
                }
                else
                {                
                    echo "<td>&nbsp;$f1<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=$posting[po_id]&thread=$posting[po_id]&aktion=show_posting&seite=$seite\">".substr((stripslashes($posting['po_titel'])),0,40)."</a></b>$f2</td>\n";
                }
...
function maske_posting($mode) {
	...
	$smilies_datei = "forum-" . $smilies_datei;
        ...
        switch ($mode) {
                ...
                case "neuer_thread":
                        ...
                        if (!$po_text) $po_text = erzeuge_fuss("");
	...
	echo "<form name=\"form\" action=\"forum.php\" method=\"post\">";
		...
		$f3."<DIV style=\"color:$farbe_text; \">($t[desc_posting])</DIV>$f4</td></tr>\n";
	...
	echo "<tr><td colspan=\"2\"><A HREF=\"$link_smilies\" TARGET=\"640_$fenster\" onMouseOver=\"return(true)\" onClick=\"neuesFenster('$link_smilies');return(false)\">$f3 <DIV style=\"color:$farbe_text; \">$t[smilies]</DIV> $f4</A></td></tr>\n";
		...
		if ($po_id == $thread)
		{
			...
			if ($po_threadgesperrt == 'Y') echo "SELECTED "; 
			echo "VALUE=\"Y\">Ja</OPTION><OPTION ";
			if ($po_threadgesperrt == 'N') echo "SELECTED "; 
			...
			if ($po_topposting == 'Y') echo "SELECTED "; 
			echo "VALUE=\"Y\">Ja</OPTION><OPTION ";
			if ($po_topposting == 'N') echo "SELECTED "; 
			echo "VALUE=\"N\">Nein</SELECT></td></tr>\n";
		}
...
function verschiebe_posting() 
{
	...
	mysql_free_result($query);
        ...
        echo "</table></td></tr>";
        echo "<tr><td><table width=\"100%\" cellspacing=\"0\" cellpadding=\"5\" border=\"0\">\n";
        echo "<tr bgcolor=\"$farbe_tabelle_zeile1\"><td>".$f1.$t['verschieben2'].$f2."</td><td>".$row2->fo_name." > ".$row2->th_name."</td></tr>";
        echo "<tr bgcolor=\"$farbe_tabelle_zeile1\"><td>".$f1.$t['verschieben3'].$f2."</td><td>";
	echo "<SELECT NAME=\"verschiebe_nach\" SIZE=\"1\">";
        while ($row3=mysql_fetch_object($query))
        {
		echo "<OPTION ";
		if ($row3->th_id == $th_id) echo "SELECTED ";
...
function show_posting() {
	...
	if (($row->u_nick!="Nobody") && ($row->u_level <> "Z")) {
		$autor=user($po_u_id,$row,$o_js,FALSE,"&nbsp;","","",TRUE,TRUE);
	} else {
		$autor = $row->u_nick;
	};
	...
	show_pfad_posting($th_id, $po_titel);
        ...
        echo "</td></tr>";
        echo "<tr bgcolor=\"$farbe_tabelle_zeile1\"><td>".$f1.$t['datum'].$po_date." ".$t['autor'].$autor.$f2."</td></tr>";
        echo "</table></td></tr>";
        echo "<tr><td>\n";
        echo "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"5\" border=\"0\">\n";
        echo "<tr bgcolor=\"$farbe_tabelle_zeile1\"><td>$f1" . $po_text . "$f2</td></tr>";
        if ($po_threadorder == "0") {
                echo "</table></td></tr></table>";
        } else {
                echo "</table></td></tr><tr><td>";
...
//Zeigt Threadbaum 
function zeige_baum(&$postings_array, &$postings, $thread, $highlight=0, $zeige_top=FALSE) {
...
//        while ($post = mysql_fetch_object($query, MYSQL_ASSOC)) {
	...
	echo "<table width=\"730\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n";
        ...
        if ($zeige_top) {
                ...
                {
                    echo "<td colspan=\"16\">&nbsp;<b><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$thread]->po_titel)),0,60) . "</font> <font size=\"-1\" color=\"red\">(gesperrt)</font></b></td>\n";
                }
                else if ($po_wahlfrei[$thread]->po_gesperrt == 'Y' and $forum_admin)
                {
                    echo "<td colspan=\"16\">&nbsp;<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=" . $po_wahlfrei[$thread]->po_id . "&thread=$thread&aktion=show_posting&seite=$seite\"><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$thread]->po_titel)),0,60) . "$f2</a> <font size=\"-1\" color=\"red\">(gesperrt)</font></td>\n";
                }
                else
                {
                    echo "<td colspan=\"16\">&nbsp;<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=" . $po_wahlfrei[$thread]->po_id . "&thread=$thread&aktion=show_posting&seite=$seite\"><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$thread]->po_titel)),0,60) . "$f2</a></td>\n";
                }
                
                if (isset($po_wahlfrei[$thread]->u_email) && $po_wahlfrei[$thread]->u_email)
                        echo "<td align=\"center\"><b>$f3<a href=\"mailto:" . $po_wahlfrei[$thread]->u_email . "\">" . $po_wahlfrei[$thread]->autor . "$f4</b></a></td>\n";
                else if (isset($po_wahlfrei[$thread]->autor) && $po_wahlfrei[$thread]->autor)
                        echo "<td align=\"center\"><b>$f3" . $po_wahlfrei[$thread]->autor . "$f4</b></td>\n";
                else
                        echo "<td align=\"center\"><b>$f3" . "$f4</b></td>\n";
                echo "<td align=\"center\"><b>$f3" . $po_wahlfrei[$thread]->po_date . "$f4</b></td>\n";
                echo "<td>&nbsp;</td></tr>\n";
	...
	$letztes = array();
		...
		if ($tiefe>=15) $tiefe=15;
                ...
                for ($i=1; $i<=$tiefe;$i++) {
                        ...
                        if (($i==$tiefe)&&($letztes[$i]))
                                ...
                                echo "<td><img src=\"pics/forum/linie.gif\" width=\"20\" height=\"20\" border=\"0\"></td>\n";
			...
			$span++;
                ...
                {
                    echo "<td colspan=\"" . (16-$span) . "\">&nbsp;<b><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$v]->po_titel)),0,(60-round($span*2.5))) . "</font> <font size=\"-1\" color=\"red\">(gesperrt)</font></b></td>\n";
                }
                else if ($po_wahlfrei[$v]->po_gesperrt == 'Y' and $forum_admin)
                {
                    echo "<td colspan=\"" . (16-$span) . "\">&nbsp;<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=" . $po_wahlfrei[$v]->po_id . "&thread=$thread&aktion=show_posting&seite=$seite\"><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$v]->po_titel)),0,(60-round($span*2.5))) . " $f2</a> <font size=\"-1\" color=\"red\">(gesperrt)</font></td>\n";
                }
                else
                {
                    echo "<td colspan=\"" . (16-$span) . "\">&nbsp;<b><a href=\"forum.php?id=$id&http_host=$http_host&th_id=$th_id&po_id=" . $po_wahlfrei[$v]->po_id . "&thread=$thread&aktion=show_posting&seite=$seite\"><font size=-1 color=\"$col\">" . substr((stripslashes($po_wahlfrei[$v]->po_titel)),0,(60-round($span*2.5))) . " $f2</a></td>\n";
                }
		...
		if (!($po_wahlfrei[$v]->u_nick)) {
                        ...
                        $userlink = user($po_wahlfrei[$v]->po_u_id,$userdata,$o_js,FALSE,"&nbsp;","","",TRUE,FALSE,29);
			...
			else
			{
                ...
                echo "<td align=\"center\"><b>$f3" . $po_wahlfrei[$v]->po_date . "$f4</b></td>\n";

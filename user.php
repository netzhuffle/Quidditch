...
case "userimport2":
	...
	if ($u_level=="S") {
		if (is_uploaded_file($HTTP_POST_FILES['userdatei']['tmp_name'])) {
			$file=fopen($HTTP_POST_FILES['userdatei']['tmp_name'],"r");
			if ($file) {
...
case "userimport":
	...
	if ($u_level=="S") {
		...
		$text= "<FORM NAME=\"userimport2\" enctype=\"multipart/form-data\" ACTION=\"user.php\" method=\"post\">\n".
			...
			"<INPUT TYPE=\"HIDDEN\" NAME=\"aktion\" VALUE=\"userimport2\">\n".
			"<TR><TD colspan=2>".$f1.htmlspecialchars($t['sonst46']).$f2."</TD></TR>\n".
			"<TR><TD colspan=2><IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=10></TD></TR>\n".
			"<TR><TD colspan=2>".$f1."&nbsp;<INPUT name=\"userdatei\" type=\"file\" SIZE=\"$eingabe_breite\">".$f2."</TD></TR>\n";

		$text.=	"<TR><TD colspan=2 ALIGN=RIGHT>".$f1."<INPUT TYPE=\"SUBMIT\" NAME=\"los\" VALUE=\"Go!\">".$f2."</TD></TR>".
			"</TABLE></FORM>\n";
		show_box2 ($box,$text,"100%");
		echo "<IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=4><BR>\n";
...
case "userloeschen":
	...
	if ($u_level=="S") {
		...
		$text= "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0>".
			"<TR><TD colspan=2>".$f1.$t['sonst49']." <A HREF=\"user.php?http_host=$http_host&id=$id&aktion=userloeschen2\">[".$t['sonst50']."]</A>".$f2."</TD></TR>\n".
			"</TABLE>\n";
		show_box2 ($box,$text,"100%");
		echo "<IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=4><BR>\n";
	...
	if ($u_level=="S") {
		...
		if (!$result || mysql_num_rows($result)==0) {
			...
			while ($row = mysql_fetch_object($result)) 
			{
				if  ($erweitertefeatures)
				{
 				     echo str_replace("\\","",htmlspecialchars($row->u_nick))."\t".
					str_replace("\\","",htmlspecialchars($row->u_name))."\t".
...
case "suche":
	...
	if (strlen($suchtext)>3 OR (isset($suchtext_eingabe) && $suchtext_eingabe=="Go!")) {
		...
		$text= "<FORM NAME=\"suche\" ACTION=\"user.php\" METHOD=POST>\n".
			...
			"<TR><TD colspan=2>".$f1.$t['sonst6'].$f2."</TD></TR>".
			"<TR><TD colspan=2>".$f1."&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"suchtext\" VALUE=\"$suchtext\" SIZE=\"$eingabe_breite\">".$f2."</TD></TR>\n";

		if ($admin) {

			// Suchformular nach IP-Adressen
			$text.= "<TR><TD colspan=2><BR>".$f1.$t['sonst30'].$f2."</TD></TR>".
				"<TR><TD ALIGN=RIGHT>".$f1.$t['sonst31'].$f2."</TD><TD>".$f1."&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"f[ip]\" VALUE=\"$f[ip]\" SIZE=\"6\">".$f2."</TD></TR>\n";

			// Liste der Gruppen ausgeben
			$text.= "<TR><TD ALIGN=RIGHT>".$f1.$t['sonst21']."</TD>\n".
		...
		$text.= "<TR><TD ALIGN=RIGHT>".$f1.$t['sonst32'].$f2."</TD><TD>".$f1."&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"f[user_neu]\" VALUE=\"$f[user_neu]\" SIZE=\"6\">&nbsp;".$t['sonst34'].$f2."</TD></TR>\n".
			"<TR><TD ALIGN=RIGHT>".$f1.$t['sonst33'].$f2."</TD><TD>".$f1."&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"f[user_login]\" VALUE=\"$f[user_login]\" SIZE=\"6\">&nbsp;".$t['sonst35'].$f2."</TD></TR>\n";

		// Suche nach User mit Homepage
		$text.= "<TR><TD ALIGN=RIGHT>".$f1.$t['sonst38']."</TD>\n".
		...
		$text.=	"<TR><TD colspan=2 ALIGN=RIGHT>".$f1."<INPUT TYPE=\"SUBMIT\" NAME=\"suchtext_eingabe\" VALUE=\"Go!\">".$f2."</TD></TR>".
			"</TABLE></FORM>\n";
		show_box2 ($box,$text,"100%");
		echo "<IMG SRC=\"pics/fuell.gif\" ALT=\"\" WIDTH=4 HEIGHT=4><BR>\n";
		...
		if ($admin)
		{
                        ...
                        if ($f['ip'] && $subquery) $subquery .= "AND u_ip_historie LIKE '%$f[ip]%' ";
                        else if ($f['ip']) $subquery = " u_ip_historie LIKE '%$f[ip]%' ";
		}

		if ($f['u_chathomepage']=="J" && $subquery) $subquery .= "AND u_chathomepage='J' ";
		else if ($f['u_chathomepage']=="J")  $subquery = " u_chathomepage='J' ";

 		if ($f['user_neu'] && $subquery) $subquery .= "AND u_neu IS NOT NULL AND date_add(u_neu, interval '$f[user_neu]' day)>=NOW() ";
 		else if ($f['user_neu']) $subquery = " u_neu IS NOT NULL AND date_add(u_neu, interval '$f[user_neu]' day)>=NOW() ";

		if ($f['user_login'] && $subquery) $subquery .= "AND date_add(u_login, interval '$f[user_login]' hour)>=NOW() ";
		else if ($f['user_login']) $subquery = " date_add(u_login, interval '$f[user_login]' hour)>=NOW() ";

		if ($u_level == "U" && $subquery) $subquery .= "AND u_level IN ('A','C','G','M','S','U') ";
		else if ($u_level == "U") $subquery = " u_level IN ('A','C','G','M','S','U') ";


		// Zur Sicherheit, falls ein Gast die Such URL direkt aufruft
		if ($u_level == "G") $subquery = " 1=2 ";
		...
		$query = $select;
		if ($where[0] == "WHERE " && $subquery) $query .= $where[0]." ".$subquery; 
		else if ($where[0] != "WHERE " && $subquery) $query .= $where[0] . " AND " . $subquery;
		else if ($where[0] != "WHERE ") $query .= $where[0];
		...
		if ($anzahl==0) {
			...
			echo "<P>".str_replace("%suchtext%",$suchtext,$t['sonst5'])."</P>\n";
		} elseif ($anzahl==1) {
			// Einen User gefunden, alle Userdaten ausgeben
			if (!isset($zeigeip)) $zeigeip = 0;
			...
			if ($anzahl>2000) {
				...
				// Mehrere User gefunden, als Tabelle ausgeben
					...
					if (isset($larr[$i]['isowner']) && $larr[$i]['isowner'] && $userdata['u_level']=="U") $larr[$i]['u_level']="B";
		...
		$text= "<FORM NAME=\"suche\" ACTION=\"user.php\" METHOD=POST>\n".
			...
			"<INPUT TYPE=\"HIDDEN\" NAME=\"aktion\" VALUE=\"suche\">\n";
		if ($admin) {
			$text.= "<TR><TD colspan=2>".$f1.$t['sonst25'].$f2."</TD></TR>";
		} else {
			$text.= "<TR><TD colspan=2>".$f1.$t['sonst7'].$f2."</TD></TR>";
		};

		$text.= "<TR><TD colspan=2>".$f1."&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"suchtext\" VALUE=\"$suchtext\" SIZE=\"$eingabe_breite\">".$f2."</TD></TR>\n";
		...
		if (!isset($f['user_neu'])) $f['user_neu'] = "";
		if (!isset($f['user_login'])) $f['user_login'] = "";
		if (!isset($f['u_chathomepage'])) $f['u_chathomepage'] = "";
...
default;
	...
	$result = mysql_query($query,$conn) or trigger_error(mysql_error(), E_USER_ERROR);
        ...
        for($i=0; $row=mysql_fetch_array($result,MYSQL_ASSOC); $i++) {
			...
			// Raumbesitzer einstellen, falls Level=User
			if ($larr[$i]['isowner'] && $userdata['u_level']=="U") $larr[$i]['u_level']="B";
	...
	if ( !$rows && $schau_raum>0 ) {
		...
		if ( $aktion!="chatuserliste" ) {
			echo "<TABLE CELLPADDING=\"2\" CELLSPACING=\"0\" BORDER=\"0\" WIDTH=\"100%\" BGCOLOR=\"$farbe_tabelle_kopf\">\n".
			     ...
			     "<TR><TD><B>".$larr[0]['r_name']."</B><BR>".( $larr[0]['r_topic'] ? $f1."Topic: ".$larr[0]['r_topic'].$f2."<BR>":"")."<BR>";
		} else {
	...
	if ($aktion!="chatuserliste") {
		...
		echo "<FORM NAME=\"raum\" ACTION=\"user.php\" METHOD=POST>\n".
			"<INPUT TYPE=\"HIDDEN\" NAME=\"id\" VALUE=\"$id\">\n".
			"<INPUT TYPE=\"HIDDEN\" NAME=\"http_host\" VALUE=\"$http_host\">\n".
			$f1."<SELECT NAME=\"schau_raum\" onChange=\"document.raum.submit()\">\n";
		...
		echo "</SELECT>".
			"<INPUT TYPE=\"SUBMIT\" NAME=\"eingabe\" VALUE=\"Go!\">".$f2.
			"</FORM>\n";

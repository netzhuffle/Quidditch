...
if ($erweitertefeatures) {
	...
	echo "<TABLE WIDTH=100% BORDER=0 CELLPADDING=2 CELLSPACING=0>\n".
		"<TR BGCOLOR=\"$farbe_tabelle_kopf2\" align=\"left\"><TD WIDTH=\"4%\">&nbsp;</TD>".
			...
			"&nbsp;&nbsp;&nbsp;&nbsp;Punkte ".
			strftime("%B",time())."</DIV></TD>".
			"<TD WIDTH=\"32%\" colspan=2><DIV style=\"color:$farbe_text; font-weight:bold;\">".
			"&nbsp;&nbsp;&nbsp;&nbsp;Punkte ".
			strftime("%Y",time())."</DIV></TD>".
			"<TD WIDTH=\"32%\" colspan=2><DIV style=\"color:$farbe_text; font-weight:bold;\">".
	...
	if ($result && mysql_num_rows($result)>0) {
		...
		$query="select u_punkte_monat as punkte,u_nick,u_id,u_level,u_punkte_gesamt,u_punkte_gruppe,u_chathomepage from user ".
			"where u_punkte_monat!=0 ".
			"and u_punkte_datum_monat=".date("n",time())." and u_punkte_datum_jahr=".date("Y",time())." and u_level != 'Z' ".
		...
		$query="select u_punkte_jahr as punkte,u_nick,u_id,u_level,u_punkte_gesamt,u_punkte_gruppe,u_chathomepage from user ".
			"where u_punkte_jahr!=0 ".
			"and u_punkte_datum_jahr=".date("Y",time())."  and u_level != 'Z' ".
	...
	if (is_array($array_user)) {
		for ($i=0; $i<$anzahl; $i++){
			echo "<TABLE WIDTH=100% BORDER=0 CELLPADDING=2 CELLSPACING=0>\n".
				"<TR BGCOLOR=\"$bgcolor\"><TD WIDTH=\"4%\" align=\"right\"><B>".$f1.($i+1).$f2.
				"</B></TD>";
			for ($j=0; $j<3; $j++){
				if (isset($array_user[$j]) && isset($array_user[$j][$i]) && $array_user[$j][$i]['punkte']) {
					$array_user[$j][$i]['u_punkte_anzeigen'] = 'Y';					
					echo "<TD WIDTH=\"8%\" align=\"right\">".$f1.$array_user[$j][$i]['punkte'].$f2."</TD><TD  WIDTH=\"24%\">".$f1.
					user($array_user[$j][$i]['u_id'],$array_user[$j][$i],TRUE).
						$f2."</TD>\n";
				} else {
